...
public class RemoveLowerCommand implements Command {
    ...
    public void execute(String[] args, BufferedReader input) throws Exception {
        ...
        System.out.println("Enter reference person (all your lower persons will be removed).");
        try {
            // read reference (same as above)
            ...
            System.out.print("Birthday (yyyy-MM-ddTHH:mm:ss, skip): ");
            ...
            System.out.print("Location Z: ");
            Integer lz = Integer.parseInt(input.readLine().trim());
            Location loc = new Location(lx, ly, lz);

            Person reference = new Person(name, coords, h, bday, hair, nat, loc);

            List<Integer> toRemove = new ArrayList<>();
            for (Person p : Main.collectionManager.getAllPersons()) {
                if (p.getOwnerID() == currentUser.getId() && p.compareTo(reference) < 0) {
                    toRemove.add(p.getId());
                }
            }
            for (int id : toRemove) {
                Main.collectionManager.removeById(id, currentUser.getId());
            }
            System.out.println("Removed " + toRemove.size() + " of your lower objects.");
        } catch (Exception e) {
